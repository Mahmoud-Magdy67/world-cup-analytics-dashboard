# smoke test - works with Athena or mock fallback
...
def test_data_functions_return_dataframes():
    from data.athena import get_teams, get_players, get_matches, get_predictions, get_data_source_status
    ...
    # Status should be either athena or mock
    assert status.mode in ["athena", "mock", "athena_error"]
    # If Athena connected, verify table counts
    if status.athena_enabled and status.tables_available:
